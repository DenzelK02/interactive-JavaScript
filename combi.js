...
function playNoise(layer) {
  let noiseFreq = random(100, 1000); 
  osc[layer].freq(noiseFreq);
...
function draw() {
  ...
  let clapThreshold = 0.1; 
  if (micLevel > clapThreshold && !clapTriggered) {
    clapTriggered = true;
    flashIntensity = 255; 
    playNoise(beatPos % layers); 
  ...
  if (frameCount % beatDur == 0 && playing) {
    ...
    rect(prevPos * box, beatPos % layers * box, box, box);
    ++beatPos;
    if (beatPos == steps) {
      beatPos = 0;
    }
  ...
  // Micro visual
  let micY = height - micLevel * 300;
  fill(255);
  ellipse(width - 30, micY, 20, 20); 
}
